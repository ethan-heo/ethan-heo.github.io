import { NormalizedNotionDatabaseRes } from "../models/normalized-notion.model"
import { NotionDatabaseParams } from "../models/notion.model"
import { NotionRepository } from "../notion.repository"

type GetNotionDatabases = UseCaseMultiParamsAndPromiseResult<NotionDatabaseParams, NormalizedNotionDatabaseRes>

/**
 * @name getNotionDatabasesUseCase
 * @description 노션 데이터베이스 목록을 조회합니다.
 */
export const getNotionDatabasesUseCase = (notionRepository: NotionRepository): GetNotionDatabases => ({
  execute: (params) => {
    return notionRepository.getNotionDatabases(params)
  },
})
